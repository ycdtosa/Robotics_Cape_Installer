...
void* imu_interrupt_handler( __unused void* ptr){ 
	...
	while(rc_get_state()!=EXITING && shutdown_interrupt_thread!=1) {
		...
		if(rc_get_state()==EXITING || shutdown_interrupt_thread==1){
			...
			if(first_run == 1) first_run = 0;
				...
				// then run user interrupt function if it's set
				imu_interrupt_func(); 
			}
			// release mutex
	...
	// lock the mutex and signal condition to let any existing rc_wait_for_dmp
	// to return
	pthread_mutex_lock( &rc_imu_read_mutex );
	pthread_cond_broadcast( &rc_imu_read_condition );
	pthread_mutex_unlock( &rc_imu_read_mutex );
...
* int rc_wait_for_dmp()
*
* This is a blocking function that returns 0 when new DMP data is ready. 
* Returns -1 if called when the imu is being shut down or -2 if called when the
* dmp is not enabled.
*******************************************************************************/
int rc_wait_for_dmp(){
	if(shutdown_interrupt_thread!=0){
		fprintf(stderr,"ERROR: call to rc_wait_for_dmp() after imu has been powered off.\n");
		return -1;
	}
	if(!dmp_en){
		fprintf(stderr,"ERROR: call to rc_wait_for_dmp() when dmp is not enabled.\n");
		return -2;
	}
