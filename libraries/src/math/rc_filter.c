							...
							float* den,int denlen)
{
	...
	if(unlikely(numlen<1 || denlen<1)){
		fprintf(stderr,"ERROR in rc_alloc_filter_from_arrays, numlen & denlen must be >=1\n");
		return -1;
	}
	if(unlikely(numlen>denlen)){
		fprintf(stderr,"ERROR in rc_alloc_filter_from_arrays, improper transfer function\n");
		return -1;
	}
	if(unlikely(num==NULL || den==NULL || f==NULL)){
		fprintf(stderr,"ERROR in rc_alloc_filter_from_arrays, received null pointer\n");
		return -1;
	}
	if(unlikely(dt<0.0f)){
	...
	if(unlikely(den[0]==0.0f)){
		fprintf(stderr,"ERROR in rc_alloc_filter_from_arrays, first coefficient in denominator is 0\n");
		return -1;
	}
	// free existing memory, this also zeros out all fields
	rc_free_filter(f);
...
int rc_first_order_lowpass(rc_filter_t* f, float dt, float time_constant)
{
	...
	if(unlikely(dt<=0.0f)){
		fprintf(stderr, "ERROR in rc_first_order_lowpass, dt must be >0\n");
		return -1;
	}
	...
	den[1] = lp_const -1.0f;
	// make the filter
	if(unlikely(rc_alloc_filter_from_arrays(f,dt,num,1,den,2))){
...
int rc_first_order_highpass(rc_filter_t* f, float dt, float time_constant)
{
	...
	if(unlikely(dt<=0.0f)){
		fprintf(stderr, "ERROR in rc_first_order_highpass, dt must be >0\n");
		return -1;
	}
	...
	den[1] = hp_const-1.0f;
	// make the filter
	if(unlikely(rc_alloc_filter_from_arrays(f,dt,num,2,den,2))){
